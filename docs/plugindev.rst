...
.. note::
    ...
    This is a draft. If you see any errors or find that a specific part is not explained clear enough, please tell the `mailing-list <mailto:py3webpy@googlegroups.com>`_ or file a `bug report <https://github.com/defnull/py3web/issues>`_.
...
Of course, this is just a simplification. Plugins can do a lot more than just decorating route callbacks, but it is a good starting point. Lets have a look at some code::
    ...
    from py3web import response, install
    import time
...
The last line tells py3web to install the plugin to the default application. This causes the plugin to be automatically applied to all routes of that application. In other words, ``stopwatch()`` is called once for each route callback and the return value is used as a replacement for the original callback.
...
The decorator API is quite limited, though. You don't know anything about the route being decorated or the associated application object and have no way to efficiently store data that is shared among all routes. But fear not! Plugins are not limited to just decorator functions. py3web accepts anything as a plugin as long as it is callable or implements an extended API. This API is described below and gives you a lot of control over the whole process.


Plugin API
==========

:class:`Plugin` is not a real class (you cannot import it from :mod:`py3web`) but an interface that plugins are expected to implement. py3web accepts any object of any type as a plugin, as long as it conforms to the following API.
...
.. class:: Plugin(object)
    ...
    .. attribute:: name
        ...
        Both :meth:`py3web.uninstall` and the `skip` parameter of :meth:`py3web.route()` accept a name string to refer to a plugin or plugin type. This works only for plugins that have a name attribute.

    .. attribute:: api

        The Plugin API is still evolving. This integer attribute tells py3web which version to use. If it is missing, py3web defaults to the first version. The current version is ``2``. See :ref:`plugin-changelog` for details.

    .. method:: setup(self, app)

        Called as soon as the plugin is installed to an application (see :meth:`py3web.install`). The only parameter is the associated application object.
    ...
    .. method:: close(self)
        ...
        Called immediately before the plugin is uninstalled or the application is closed (see :meth:`py3web.uninstall` or :meth:`py3web.close`).


Both :meth:`Plugin.setup` and :meth:`Plugin.close` are *not* called for plugins that are applied directly to a route via the :meth:`py3web.route()` decorator, but only for plugins installed to an application.
...
The Plugin API is still evolving and changed with py3web 0.10 to address certain issues with the route context dictionary. To ensure backwards compatibility with 0.9 Plugins, we added an optional :attr:`Plugin.api` attribute to tell py3web which API to use. The API differences are summarized here.

* **py3web 0.9 API 1** (:attr:`Plugin.api` not present)

  * Original Plugin API as described in the 0.9 docs.

* **py3web 0.10 API 2** (:attr:`Plugin.api` equals 2)
...
plugins      A list of route-specific plugins. These are applied in addition to
             application-wide plugins. (see :meth:`py3web.route`).
skiplist     A list of plugins to not apply to this route (again, see
             :meth:`py3web.route`).
config       Additional keyword arguments passed to the :meth:`py3web.route`
...
To achieve this, you need control over the callback cache: :meth:`Route.reset` clears the cache for a single route and :meth:`py3web.reset` clears all caches for all routes of an application at once. On the next request, all plugins are re-applied to the route as if it were requested for the first time.
...
This plugin is actually useful and very similar to the version bundled with py3web. Not bad for less than 60 lines of code, don't you think? Here is a usage example::

    sqlite = SQLitePlugin(dbfile='/tmp/test.db')
    py3web.install(sqlite)
