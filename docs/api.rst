...
This is a mostly auto-generated API. If you are new to py3web, you might find the
...
The :class:`Response` class stores the HTTP status code as well as headers and cookies that are to be sent to the client. Similar to :data:`py3web.request` there is a thread-local :data:`py3web.response` instance that can be used to adjust the `current` response. Moreover, you can instantiate :class:`Response` and return it from your request handler. In this case, the custom instance overrules the headers and cookies defined in the global one.
...
The following two classes can be raised as an exception. The most noticeable difference is that py3web invokes error handlers for :class:`HTTPError`, but not for :class:`HTTPResponse` or other response types.
...
All template engines supported by :mod:`py3web` implement the :class:`BaseTemplate` API. This way it is possible to switch and mix template engines without changing the application code at all.
...
To use :class:`MakoTemplate` as your default template engine, just import its specialised decorator and render function::
  ...
  from py3web import mako_view as view, mako_template as template
